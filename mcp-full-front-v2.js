...
function buildMcp() {
  ...
  mcp.registerTool("roberta_migrate", {
    ...
    inputSchema: z.object({ migrationId: z.string().min(1), description: z.string().min(1), sql: z.string().min(1), bridgeId: z.string().optional() }),
    annotations: de
  }, async ({ migrationId, description, sql, bridgeId }) => result(await callBridge("pg.roberta.migrate", { migrationId, description, sql }, bridgeId || "")));

  mcp.registerTool("session_bootstrap", {
    title: "Bootstrap AUTSYS",
    description: "Carica da ROBERTA catalogo progetti, progetto corrente, stato e capacità all'inizio della sessione.",
    inputSchema: z.object({ scope: z.enum(["GENERAL", "PROJECT"]), projectName: z.string().optional(), bridgeId: z.string().optional() }),
    annotations: ro
  ...
  mcp.registerTool("project_register", {
    ...
    annotations: wr
  }, async ({ bridgeId, projectName, ...rest }) => result(await callBridge("project.register", { displayName: projectName, ...rest }, bridgeId || "")));
  ...
  mcp.registerTool("bridge_update_stage", {
    ...
    inputSchema: z.object({ request: z.record(z.any()), bridgeId: z.string().optional() }),
    annotations: wr
  }, async ({ request, bridgeId }) => result(await callBridge("bridge.update.stage", request, bridgeId || "")));
  ...
  mcp.registerTool("bridge_update_apply", {
    ...
    inputSchema: z.object({ request: z.record(z.any()), bridgeId: z.string().optional() }),
    annotations: de
  }, async ({ request, bridgeId }) => result(await callBridge("bridge.update.apply", request, bridgeId || "")));
...
app.post("/mcp", async (req, res) => {
  ...
  try {
    ...
    if (!res.headersSent) {
      res.status(500).json({ jsonrpc: "2.0", error: { code: -32603, message: "MCP internal error" }, id: null });
    }
  }
});
...
app.listen(PORT, "127.0.0.1", () => {
  console.log(`AUTSYS MCP FULL FRONT ${VERSION} listening on ${PORT}; backend=${BACKEND_PORT}`);
});
